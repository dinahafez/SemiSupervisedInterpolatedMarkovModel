...
frame5 <- read.table(args[5])
ordered_frame5 <- frame5#[order(-frame$V2),]
ordered_frame5_nohead <- ordered_frame5[ ,-1]
rev_ordered_frame5 <- as.data.frame(t(ordered_frame5_nohead))
averaged5 <- rowMeans(rev_ordered_frame5)

frame6 <- read.table(args[6])
ordered_frame6 <- frame6#[order(-frame$V2),]
ordered_frame6_nohead <- ordered_frame6[ ,-1]
rev_ordered_frame6 <- as.data.frame(t(ordered_frame6_nohead))
averaged6 <- rowMeans(rev_ordered_frame6)

averaged_frame <- data.frame(x = seq(-2000, 1999, 10),  "H3K4me1" = averaged4, "H3K27ac"=averaged1,  "H3K9ac" = averaged6, "H3K4me3"=averaged2, "H3K27me3" = averaged3, "H3K9me3" = averaged5,check.names=FALSE)
melted_averaged_frame <- melt(averaged_frame, id="x")
head(melted_averaged_frame)

tiff(filename = paste0(args[1],"_multi.tiff"), width = 1800, height = 1600, res = 5)
ggplot(melted_averaged_frame, aes(x, value, colour=variable)) + geom_line(size=100) + scale_color_manual(values=c("H3K27ac" = "#D55E00", "H3K4me1" = "#009E73", "H3K4me3" = "#56B4E9", "H3K27me3" = "#CC79A7", "H3K9me3" = "#F0E442", "H3K9ac" = "#999999")) + xlab("Distance From DHSs midpoints") + ylab("peak coverage score") + coord_cartesian(xlim = c(-1500,1500), ylim =c(0,0.4) ) + theme(axis.line=element_blank(), axis.text.x=element_text(size=1000, face="bold"), axis.text.y=element_text(size=1000, face="bold"), axis.ticks.y=element_line(size=40), axis.ticks.x=element_line(size=40), axis.ticks.margin=unit(1, "cm"), axis.title.x=element_blank(), axis.title.y=element_blank(), axis.ticks.length=unit(3, "cm"), plot.margin=unit(c(20,10,10,10), "cm"), panel.border=element_blank(), panel.grid=element_blank(), panel.margin=element_blank(), panel.background=element_blank(), legend.position=c(1,1), legend.key.size=unit(50, "cm"), legend.text=element_text(size=750), legend.key=element_blank(), legend.justification=c(1,1), strip.background=element_blank(), legend.box.just="right")
