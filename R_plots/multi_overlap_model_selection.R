...
for(i in 11:39)
{
  ...
  file3 = paste(i,"_pos_10_single_merged_stage2_all_ubiquitous_all_single_merged_stage2_all_Classifier.ROC", sep="")
  if (file.exists(file3))
  {
    auc = read.table(file3, header=FALSE)$V3
    auc2 = as.numeric(as.character(auc))
...
mtext("stage1", side = 1, at=-0,line=1)
mtext("stage2", side = 1, at=1,line=1)
#mtext("stage2_12", side = 1, at=2)
mtext("stage1+TSS", side = 1, at=2.0,line=1)
mtext("stage2+TSS", side = 1, at=3.0, line=1)
