...
sub readGenesInStudy
{
	for (my $i=15; $i <= 38; $i++)
	{
		if (($i!= 19)&&($i!= 21)&&($i!= 22)&&($i!= 30)&&($i!= 31)&&($i!= 18)&&($i!= 27))
		{
...
sub readGenesInHiC
{
	my $file = $resultDrive."partnerTogene_intersect_DHS_genes_in_study_stage1.bed";
	open IN, "<$file" or die "Can't open file:$file";
	while (my $line = <IN>)
	{
...
{
	my $file = $resultDrive.$ARGV[0]; 
	open IN, "<$file" or die "Can't open file:$file";

	my $fileout =$resultDrive.$ARGV[0].".filtered";
	open OUT, ">$fileout" or die "Can't open file:$fileout";
	while (my $line = <IN>)
	{
