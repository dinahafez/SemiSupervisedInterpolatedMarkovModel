...
sub readGenesInHiC
{
	my $file = $dataDrive."partnerTogene_intersect_DHS_genes_in_study.bed";
	open IN, "<$file" or die "Can't open file:$file";
	while (my $line = <IN>)
	{
...
{
	my $file = $resultDrive.$ARGV[0]; #model_not_validated_HiC_intersect_HiC.bed ";
	open IN, "<$file" or die "Can't open file:$file";

	my $fileout =$resultDrive.$ARGV[0].".filtered";#"model_not_validated_HiC_intersect_HiC_and_genes.bed";
	open OUT, ">$fileout" or die "Can't open file:$fileout";
	while (my $line = <IN>)
	{
